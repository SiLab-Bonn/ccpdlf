...
class ccpdlf_cppm(Dut):
    def __init__(self,conf=""):
        if conf=="":
            conf="ccpdlfA_cppm.yaml"
    ...
    def get_status(self):
        stat={"Vdda": self['CCPD_Vdda'].get_voltage(unit='V'),
            ...
            "INJ_WIDTH":self["CCPD_PULSE_INJ"]["WIDTH"],
            "INJ_DELAY":self["CCPD_PULSE_INJ"]["DELAY"],
            "INJ_REPEAT":self["CCPD_PULSE_INJ"]["REPEAT"],
            "INJ_EN":self["CCPD_PULSE_INJ"]["EN"],
            "GATE_WIDTH":self["CCPD_PULSE_GATE"]["WIDTH"],
            "GATE_DELAY":self["CCPD_PULSE_GATE"]["DELAY"],
            "GATE_REPEAT":self["CCPD_PULSE_GATE"]["REPEAT"],
            "GATE_EN":self["CCPD_PULSE_GATE"]["EN"]
            } 
    ...
    def set_tdc_trig(self,delay=10,width=100):
        # set gpio
        self['rx']['FE'] = 0
        self['rx']['TLU'] = 0
        self['rx']['CCPD_TDC'] = 1
        ...
        self["CCPD_PULSE_GATE"]["REPEAT"]=1
        self["CCPD_PULSE_GATE"]["DELAY"]=delay
        self["CCPD_PULSE_GATE"]["WIDTH"]=width
        ...
        s="width:%d delay:%d"%(width,delay)
        self.logger.info(s)
        
    def get_tdc_now(self):
...
class ccpdlfB_cppm(ccpdlf):
    def __init__(self,conf=""):
        self.init_log()
        if conf=="":
            conf="ccpdlf_cppm.yaml"
        super(ccpdlf, self).__init__(conf)
        self.debug=2
